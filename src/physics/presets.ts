...
import type { BodySpec, GravityMode, Preset } from './types.ts';
...
/** GAMEPLAY APPROXIMATION for unnamed/procedural bodies only. */
function approximatePhysicalRadiusAU(mass: number, opts: PlanetOpts): number {
  if (opts.physicalRadius != null) return opts.physicalRadius;
  if (opts.key && PHYSICAL_RADIUS_AU[opts.key] != null) return PHYSICAL_RADIUS_AU[opts.key];
  if (opts.isBlackHole) return R_SCHWARZSCHILD_SUN_AU * Math.max(mass, 0);
  if (opts.isStar) return R_SUN_AU * Math.pow(Math.max(mass, 1e-6), 0.8);
  const rho = mass > 1e-4 ? 1300 : 5500;
  const M_kg = mass * M_SUN_KG;
  const R_m = Math.cbrt((3 * M_kg) / (4 * Math.PI * rho));
  return R_m / AU_M;
}
...
export function orbiting(
  ...
  M = 1,
  opts: PlanetOpts = {},
  center: { x: number; y: number; z?: number; vx: number; vy: number; vz?: number } = {
  ...
  if (Math.abs(i) > 1e-12) {
    ...
    y = center.y + st.y;
    z = (center.z ?? 0) + st.z;
    vx = center.vx + st.vx * vMul;
    vy = center.vy + st.vy * vMul;
    vz = (center.vz ?? 0) + st.vz * vMul;
  } else {
    const v = circularSpeed(M, a) * vMul;
  ...
  const physicalRadius = approximatePhysicalRadiusAU(mass, opts);
  return {
    name,
    key: opts.key,
    mass,
    physicalRadius,
    renderRadius: opts.renderRadius ?? renderRadius,
    color,
    x, y, z, vx, vy, vz,
    isStar: opts.isStar,
...
export const SUN: BodySpec = {
  ...
  key: 'sun',
  mass: 1,
  physicalRadius: PHYSICAL_RADIUS_AU.sun,
  renderRadius: 0.09,
  x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0,
  color: '#ffd27a',
  isStar: true,
...
const TEST_PARTICLE: PlanetOpts = { noCollide: true, gravityMode: 'test-particle' };
...
function trojans(jupiterAngleDeg: number, jupiterA: number, countPerCamp: number, seed = 77): BodySpec[] {
  ...
  for (let c = 0; c < 2; c++) {
    ...
    for (let i = 0; i < countPerCamp; i++) {
      const a = jupiterA * (0.985 + rng(st) * 0.03);
...
function blackHole(name: string, mass: number, renderRadius: number, x: number, y: number, vx: number, vy: number, key?: string): BodySpec {
  return {
    name, key, mass,
    physicalRadius: R_SCHWARZSCHILD_SUN_AU * mass,
    renderRadius,
    x, y, z: 0, vx, vy, vz: 0,
    color: '#a78bfa',
    isBlackHole: true,
...
function grandBodies(): BodySpec[] {
  ...
  const moons: BodySpec[] = [
    ...
    ...moonsOf(pluto, [{ name: '冥卫一·卡戎', key: 'charon', a: 0.0006, mass: 7.9e-10, radius: 0.00007, color: '#b9b0a4', angle: 90 }]),
  ];
  return balance([
...
export const PRESETS: Preset[] = [
  ...
  },
  {
    ...
    bodies: (() => {
      const m = 0.6;
      const d = 1.0;
      const v = circularSpeed(2 * m, d) / 2;
      const stars: BodySpec[] = [
        { name: '主星 A', key: 'starA', mass: m, physicalRadius: R_SUN_AU * Math.pow(m, 0.8), renderRadius: 0.07, x: -d / 2, y: 0, z: 0, vx: 0, vy: -v, vz: 0, color: '#ffb36b', isStar: true },
        { name: '伴星 B', key: 'starB', mass: m, physicalRadius: R_SUN_AU * Math.pow(m, 0.8), renderRadius: 0.06, x: d / 2, y: 0, z: 0, vx: 0, vy: v, vz: 0, color: '#8fd3ff', isStar: true },
      ];
      const M = 2 * m;
      return [
        ...stars,
  ...
  },
  {
    ...
    bodies: (() => {
      ...
      const rStar = R_SUN_AU;
      return [
        { name: '恒星 α', key: 'alpha', mass: 1, physicalRadius: rStar, renderRadius: 0.045, x: p[0], y: p[1], z: 0, vx: v[0] * k, vy: v[1] * k, vz: 0, color: '#ff8a65', isStar: true },
        { name: '恒星 β', key: 'beta', mass: 1, physicalRadius: rStar, renderRadius: 0.045, x: -p[0], y: -p[1], z: 0, vx: v[0] * k, vy: v[1] * k, vz: 0, color: '#7fd1ff', isStar: true },
        { name: '恒星 γ', key: 'gamma', mass: 1, physicalRadius: rStar, renderRadius: 0.045, x: 0, y: 0, z: 0, vx: -2 * v[0] * k, vy: -2 * v[1] * k, vz: 0, color: '#fff59d', isStar: true },
      ] as BodySpec[];
    })(),
  },
  {
  ...
  },
  {
    ...
    epoch: DEFAULT_EPOCH,
    bodies: balance([
      { name: '木星', key: 'jupiter', mass: 9.55e-4, physicalRadius: PHYSICAL_RADIUS_AU.jupiter, renderRadius: 0.008, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, color: '#d9a066' },
  ...
  },
  {
    ...
    dt: 2e-5,
    bodies: balance([
      { name: 'TRAPPIST-1', key: 'trappist1', mass: 0.09, physicalRadius: PHYSICAL_RADIUS_AU.trappist1, renderRadius: 0.035, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, color: '#ff7a5c', isStar: true },
...
export function generateRandomSystem(seed: number): Preset {
  ...
  if (binary) {
    ...
    bodies.push(
      { name: '主星 A', mass: m, physicalRadius: R_SUN_AU * Math.pow(m, 0.8), renderRadius: 0.07, x: -d / 2, y: 0, z: 0, vx: 0, vy: -v, vz: 0, color: '#ffb36b', isStar: true },
      { name: '伴星 B', mass: m, physicalRadius: R_SUN_AU * Math.pow(m, 0.8), renderRadius: 0.06, x: d / 2, y: 0, z: 0, vx: 0, vy: v, vz: 0, color: '#8fd3ff', isStar: true },
    );
  } else {
    bodies.push({ name: '恒星', mass: starMass, physicalRadius: R_SUN_AU * Math.pow(starMass, 0.8), renderRadius: 0.09, x: 0, y: 0, z: 0, vx: 0, vy: 0, vz: 0, color: '#ffd27a', isStar: true });
  }
  const nPlanets = 2 + Math.floor(rnd() * 6);
  let a = 0.3 + rnd() * 0.3;
  for (let i = 0; i < nPlanets; i++) {
    if (i > 0) a *= 1.4 + rnd() * 0.6;
    const mass = Math.pow(10, -7 + rnd() * 4);
    bodies.push(orbiting(`行星-${i + 1}`, a, mass, radiusForMass(mass), RANDOM_COLORS[i % RANDOM_COLORS.length], rnd() * 360, starMass));
  }
  let outer = a;
  if (rnd() < 0.3) {
    const count = 40 + Math.floor(rnd() * 41);
    const rMin = a * 1.4;
    const rMax = a * 2.0;
    for (let i = 0; i < count; i++) {
  ...
  if (binary) {
    ...
    for (let i = 2; i < bodies.length; i++) {
      ...
      pz += bodies[i].mass * (bodies[i].vz ?? 0);
    }
    const mTot = bodies[0].mass + bodies[1].mass;
    for (let i = 0; i < 2; i++) {
      bodies[i].vx -= px / mTot;
      bodies[i].vy -= py / mTot;
      bodies[i].vz = (bodies[i].vz ?? 0) - pz / mTot;
    }
  } else {
    balance(bodies, 0);
  }
...
export const LAUNCH_TYPES: LaunchType[] = [
  { id: 'asteroid', name: '小行星', mass: 1e-11, physicalRadius: 3.3e-7, renderRadius: 0.006, color: '#a8a29e' },
  { id: 'comet', name: '彗星', mass: 5e-11, physicalRadius: 1.0e-7, renderRadius: 0.007, color: '#c7f5ff' },
  { id: 'rocky', name: '岩质行星', mass: 3e-6, physicalRadius: PHYSICAL_RADIUS_AU.earth, renderRadius: 0.02, color: '#7dd3a0' },
  { id: 'giant', name: '气态巨行星', mass: 1e-3, physicalRadius: PHYSICAL_RADIUS_AU.jupiter, renderRadius: 0.05, color: '#f0a06a' },
  { id: 'dwarf', name: '红矮星', mass: 0.15, physicalRadius: R_SUN_AU * Math.pow(0.15, 0.8), renderRadius: 0.06, color: '#ff7a5c', isStar: true },
  { id: 'star', name: '恒星', mass: 1, physicalRadius: R_SUN_AU, renderRadius: 0.09, color: '#ffe082', isStar: true },
  { id: 'blackhole', name: '黑洞', mass: BLACK_HOLE_LAUNCH_MASS, physicalRadius: R_SCHWARZSCHILD_SUN_AU * BLACK_HOLE_LAUNCH_MASS, renderRadius: 0.035, color: '#a78bfa', isBlackHole: true },
];
...
export function rogueStar(center: { x: number; y: number }): BodySpec {
  ...
  return {
    ...
    mass: 0.4,
    physicalRadius: R_SUN_AU * Math.pow(0.4, 0.8),
    renderRadius: 0.065,
    x, y, z: 0, vx: (dx / len) * speed, vy: (dy / len) * speed, vz: 0,
    color: '#ff6b6b',
    isStar: true,
  };
}
